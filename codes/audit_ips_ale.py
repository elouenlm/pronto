...
class IpsFullAuditor:
    ...
    def prepare_data(self):
        ...
        # On cible la session la plus récente et les données exploitables
        df_brevet = df_brut[df_brut['Session'] == 2024].copy()
        
        # Nettoyage des colonnes cibles et contextuelles
        ...
        print("\n📊 Statistiques descriptives :")
        print(f"   IPS : {self.df['IPS'].min():.1f} → {self.df['IPS'].max():.1f} (moyenne : {self.df['IPS'].mean():.1f})")
        print(f"   Note Écrit : {self.df['Note_Ecrit'].min():.1f} → {self.df['Note_Ecrit'].max():.1f} (moyenne : {self.df['Note_Ecrit'].mean():.1f})")
        print(f"   Public : {self.df['Is_Public'].sum()} | Privé : {(~self.df['Is_Public'].astype(bool)).sum()}")
    ...
    def run_ale_analysis(self):
        ...
        # Calcul de l'importance (pour voir si l'IPS domine vraiment)
        importances = model.feature_importances_
        print("\n--- Importance des variables pour le modèle ---")
        for f, imp in zip(features, importances):
            print(f"📍 {f}: {imp:.1%}")

        print("\n📊 Génération de la courbe ALE pour l'IPS...")
        # L'ALE isole l'effet de l'IPS en neutralisant le Secteur et la Taille (Candidats)
        ale_eff = ale(X=X, model=model, feature=['IPS'], grid_size=40, include_CI=True)
        
        # PyALE crée automatiquement le plot, on le personnalise après
        fig = plt.gcf()
        fig.set_size_inches(10, 6)
        ax = plt.gca()
        ax.set_title("Effet Local Accumulé (ALE) de l'IPS sur la Note au Brevet\n(Après neutralisation de l'effet Secteur et Taille)", fontsize=13)
        ax.set_xlabel("Indice de Position Sociale (IPS)")
        ax.set_ylabel("Impact sur la note (en points)")
        ax.axhline(0, color='black', linewidth=1, alpha=0.3) # Ligne de base
